...
import VolunteerForm from "@frontend/components/services/VolunteerForm";
import { homeBtnOutlineClass, homeSectionClass } from "@frontend/components/home/homeUi";
...
import { volunteerContent } from "@frontend/content/VolunteerContent";
import { ArrowUpRight, Check } from "lucide-react";

export default function VolunteerPage() {
  const { hero, intro, opportunitiesSection, opportunities, form, cta } = volunteerContent;
    ...
    <div className="overflow-x-hidden bg-background">
      ...
      <section className={`${homeSectionClass} border-y border-border/80 bg-surface`}>
        <SectionContainer>
          <MotionSection>
            <SectionHeading
              heading={opportunitiesSection.heading}
              subheading={opportunitiesSection.subheading}
            />
          </MotionSection>
            ...
            {opportunities.map((item) => (
              <MotionItem key={item.id}>
                <article className="flex h-full flex-col overflow-hidden rounded-3xl border border-border/80 bg-background shadow-premium">
                  <div className="relative aspect-[3/4] bg-section-warm">
                    <ZoomableImage
                      src={item.imageSrc}
                      alt={item.imageAlt}
                      fill
