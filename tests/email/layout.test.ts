...
import { detailsFromText, escapeHtml, renderCiuEmail } from "@email/layout";
...
describe("email layout", () => {
  it("renders a webpage-style CIU message with the hosted logo", () => {
    ...
    expect(html).toContain("cid:ciu-logo");
    ...
    expect(html).not.toContain("cid:ciu-poster");
  });

  it("can place an optional poster in the message body", () => {
    const html = renderCiuEmail({
      title: "Picnic",
      paragraphs: ["Join us Saturday."],
      heroImage: { src: "cid:ciu-poster", alt: "Picnic poster" },
    });
    expect(html).toContain('src="cid:ciu-poster"');
    expect(html).toContain("Picnic poster");
  });
