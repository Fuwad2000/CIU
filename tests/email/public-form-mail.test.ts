...
import { CIU_INTERNAL_NOTIFICATION_TO } from "@email/form-mail";
...
vi.mock("@backend/outreach/additional", () => ({
  rememberContactPerson: vi.fn(async () => true),
}));
...
describe("public form emails", () => {
  ...
  it("does not email when a membership email is already stored", async () => {
    const { DuplicateEmailError } = await import("@backend/sql/sql-store");
