...
export default function OutreachCampaignEditor({ campaign }: { campaign?: OutreachCampaign }) {
  ...
  const confirmSend = async () => {
    ...
    try {
      const saved = await saveDraft();
      if (!saved) return;
      await adminFetch(`/api/admin/outreach/campaigns/${saved.id}/send`, { method: "POST" });
              ...
              <div className="space-y-2">
                {audiences.map((audience) => {
                  ...
                  return (
                    <label
                      key={audience.id}
                      className="flex cursor-pointer items-start gap-3 rounded-2xl border border-border bg-background px-4 py-3"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
          <div className="w-full max-w-lg rounded-3xl border border-border bg-surface p-6 shadow-xl">
            ...
            <p className="mt-4 text-sm text-muted">
              Email delivery is not connected yet. Confirming will check permissions and recipient counts, then stop
              before sending mail.
            </p>
