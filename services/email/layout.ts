...
export const EMAIL_LOGO_CID = "ciu-logo";
export { EMAIL_POSTER_CID } from "@shared/poster-image";
...
export function renderCiuEmail(content: CiuEmailContent) {
  ...
  const hero = content.heroImage
    ? `<img src="${escapeHtml(content.heroImage.src)}" alt="${escapeHtml(content.heroImage.alt ?? "Poster")}" width="544" style="display:block;width:100%;max-width:544px;height:auto;margin:0 0 22px;border:0;border-radius:12px;" />`
    : "";
            ...
            <tr>
              <td style="background:${BRAND_DARK};padding:28px 28px 22px;text-align:center;">
                <a href="${home}" style="text-decoration:none;">
                  <img src="${emailLogoSrc()}" width="72" height="72" alt="${escapeHtml(siteContent.logoAlt)}" style="display:block;margin:0 auto 14px;border:0;border-radius:18px;background:${WHITE};padding:6px;" />
                </a>
